    ...
    pub fn play_replay_move(&mut self) -> ReplayResponse {
            ...
            && self.player_moves_played_offset == 0
        {
            ...
            let played_piece = match &played_move {
                Move::BasicMove { piece, .. } => to_tanton_piece(piece, self.is_white_turn),
                _ => {
            ...
            let destination = match &played_move {
                Move::BasicMove { to, .. } => SQ::make(
                    to_tanton_file(to.file()).unwrap(),
                    to_tanton_rank(to.rank()).unwrap(),
                ),
                Move::CastleKingside => {
                    if self.is_white_turn {
                        SQ::make(File::H, Rank::R1)
                    } else {
                        SQ::make(File::H, Rank::R8)
                    }
                }
                Move::CastleQueenside => {
                    if self.is_white_turn {
                        ...
                        SQ::make(File::A, Rank::R8)
                    }
                }
            };
            let (src_col, src_row) = match &played_move {
                Move::BasicMove { from, .. } => {
                    (to_tanton_file(from.file()), to_tanton_rank(from.rank()))
                }
                Move::CastleKingside => (
                    Some(File::E),
                    if self.is_white_turn {
            ...
            match self.active_game.move_piece_by_type(
                ...
                Err(_) => {
                    comment =
                        Some("Error playing replay move, please check your PGN's validity".into())
                }
            }
...
fn to_tanton_piece(piece: &LocalPiece, is_white_turn: bool) -> Piece {
    if is_white_turn {
        match piece {
...
fn to_tanton_rank(rank: Option<LocalRank>) -> Option<Rank> {
    match rank {
...
fn to_tanton_file(file: Option<LocalFile>) -> Option<File> {
    if file.is_some() {
        match file {
